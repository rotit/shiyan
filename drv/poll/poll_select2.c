...
static wait_queue_head_t my_queue_read;
static wait_queue_head_t my_queue_write;
		...
		loff_t *ppos)
{
	...
	if(down_interruptible(&sem))
		return -ERESTARTSYS;
	printk("jinmin-- buffer_char_count=%d\n",buffer_char_count);
	wait_event_interruptible(my_queue_read, buffer_char_count > 0);
	printk("after read wake\n");
	if (buffer_char_count > 0)
	{
		printk("in >0\n");
		printk("read copy to user\n");
		...
		size_t count, loff_t *ppos)
{
	...
	if(down_interruptible(&sem))
		return -ERESTARTSYS;
	printk("jinmin-- before,write,buff char count=%d\n",buffer_char_count);
	wait_event_interruptible(my_queue_write, buffer_char_count == 0);
	...
	printk("jinmin-- after,write,buff char count=%d\n",buffer_char_count);
...
unsigned int demo_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	if(down_interruptible(&sem))
		return -ERESTARTSYS;
	printk("jinmin--- in poll\n");
	poll_wait(filp, &my_queue_write, wait);
	poll_wait(filp, &my_queue_read, wait);
	// read
	if(buffer_char_count > 0)
	{
...
static int __init demo_init(void)
{
	...
	sema_init(&sem, 1);
	printk("demo_init_success poll2\n");
